...
- 管理页平铺展示用户自有订阅源，不展示 `rsshub-doc-*`
- 管理页弱化分类，分类只是订阅源属性；顶部提供类型筛选，不再使用左侧分类目录
- 管理页新增和编辑都走同一个弹窗表单
- RSSHUB 是独立一级页面，用来展示从 RSSHub 官方文档同步的 `rsshub-doc-*` 模板路由
- RSSHUB 页复用管理页两栏布局，但不提供用户分类创建入口
...
- `rsshub-doc-*` 属于文档同步生成项，再次同步时应更新这些项但默认保持停用，并且应留在 RSSHUB 模块，不要混入“管理订阅源”
